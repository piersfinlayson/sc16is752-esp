...
void ICACHE_FLASH_ATTR brzo_i2c_setup_info(brzo_i2c_info *info)
{
	...
	PIN_FUNC_SELECT(info->sda_pin_mux, info->sda_pin_func);
	PIN_FUNC_SELECT(info->scl_pin_mux, info->scl_pin_func);

	GPIO_REG_WRITE(GPIO_PIN_ADDR(GPIO_ID_PIN(info->sda_pin)), GPIO_REG_READ(GPIO_PIN_ADDR(GPIO_ID_PIN(info->sda_pin))) | GPIO_PIN_PAD_DRIVER_SET(GPIO_PAD_DRIVER_ENABLE)); //open drain;
	GPIO_REG_WRITE(GPIO_ENABLE_ADDRESS, GPIO_REG_READ(GPIO_ENABLE_ADDRESS) | (1 << info->sda_pin));
	GPIO_REG_WRITE(GPIO_PIN_ADDR(GPIO_ID_PIN(info->scl_pin)), GPIO_REG_READ(GPIO_PIN_ADDR(GPIO_ID_PIN(info->scl_pin))) | GPIO_PIN_PAD_DRIVER_SET(GPIO_PAD_DRIVER_ENABLE)); //open drain;
	GPIO_REG_WRITE(GPIO_ENABLE_ADDRESS, GPIO_REG_READ(GPIO_ENABLE_ADDRESS) | (1 << info->scl_pin));

	ETS_GPIO_INTR_ENABLE();

	info->sda_bitmask = (uint16_t)(1 << info->sda_pin);
	info->scl_bitmask = (uint16_t)(1 << info->scl_pin);
#endif
